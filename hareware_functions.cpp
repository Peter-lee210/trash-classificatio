...
void layer_CONV_3x3(hls::stream<DATA_STREAM> &ext_in_data, hls::stream<DATA_STREAM> &ext_out_data,
                    ...
                    volatile DATA_SW *ext_bn_scale, volatile DATA_SW *ext_bn_shift,
...
void PEs(hls::stream<DATA_STREAM> &ext_in_data, hls::stream<DATA_STREAM> &ext_out_data,
         ...
         volatile DATA_SW *ext_bn_scale, volatile DATA_SW *ext_bn_shift,
         volatile DATA_SW *ext_w_fc, volatile DATA_SW *ext_b_fc,
         DATA_HW tile_index[3], DATA_SW quant[4], DATA_HW info[size_info], CALL_DATA call_PE, DATA_SW type_layer);
...
//HARDWARE FUNCTIONS
//
ACT_CONV running_mean;      // Giá trị trung bình
ACT_CONV running_variance;  // Phương sai
const ACT_CONV epsilon = 1e-5; // Hằng số nhỏ để tránh chia cho 0
//TILING
void layer_CONV_3x3(hls::stream<DATA_STREAM> &ext_in_data, hls::stream<DATA_STREAM> &ext_out_data,
                    volatile DATA_SW *ext_w_conv, volatile DATA_SW *ext_b_conv,
                    volatile DATA_SW *ext_bn_scale, volatile DATA_SW *ext_bn_shift,
    ...
    ACT_CONV running_mean = 0;      // Giá trị trung bình
    ACT_CONV running_variance = 1;  // Phương sai
    const ACT_CONV epsilon = 1e-5;   // Hằng số nhỏ để tránh chia cho 0
    ...
    // Đọc các tham số Batch Norm
    DATA_HW bn_scale = ext_bn_scale[info[14]]; // Thay đổi chỉ số theo cách bạn lưu trữ
    DATA_HW bn_shift = ext_bn_shift[info[14]];
        ...
        LOOP_map_k_3X3: for (DATA_HW k_tile = 0; k_tile < info[1]; k_tile++) {
            ...
            LOOP_map_l_3X3: for (DATA_HW l_tile = 0; l_tile < info[1]; l_tile++) {
                ...
                if (k_tile > 0 && k_tile < info[10] && l_tile > 0 && l_tile < info[10] && stride_x && stride_y) {
                    ...
                    // Batch Norm
                    temp = (temp - running_mean) / sqrt(static_cast<float>(running_variance + epsilon)); // Thay đổi các giá trị này theo cách bạn lưu trữ
                    temp = temp * bn_scale + bn_shift; // Áp dụng scale và shift
...
void layer_expansion_projection(hls::stream<DATA_STREAM> &ext_in_data, hls::stream<DATA_STREAM> &ext_out_data,
    ...
    LOOP_depth_expproj: for (DATA_HW i_tile = 0, j_tile = 0, pos_weight = 0, distance = 0; i_tile < tile_index[0]; j_tile++) {
        ...
        if (distance == i_conv[pos_weight]) {
            ...
            // Đọc các tham số Batch Norm
            DATA_HW bn_scale = ext_bn_scale[i_tile]; // Thay đổi chỉ số theo cách bạn lưu trữ
            DATA_HW bn_shift = ext_bn_shift[i_tile];
            ...
            if (info[5] && j_tile == info[6]) {
                ...
                if (read_data[j_tile] == 0) {
                    // Loop length map
                    LOOP_mapk_w1_expproj: for (DATA_HW k_tile = 0; k_tile < tile_index[2]; k_tile++) {
                        #pragma HLS LOOP_TRIPCOUNT min=7 max=28
                        LOOP_mapl_w1_expproj: for (DATA_HW l_tile = 0; l_tile < HLS_tile_map; l_tile++) {
                            #pragma HLS dependence variable=out_map inter false
                            #pragma HLS dependence variable=temp_map inter false
                            #pragma HLS PIPELINE II=1
                    ...
                    // Loop length map
                    LOOP_mapk_w0_expproj: for (DATA_HW k_tile = 0; k_tile < tile_index[2]; k_tile++) {
                        #pragma HLS LOOP_TRIPCOUNT min=7 max=28
                        #pragma HLS PIPELINE II=1
                        LOOP_mapl_w0_expproj: for (DATA_HW l_tile = 0; l_tile < HLS_tile_map; l_tile++) {
                            #pragma HLS dependence variable=out_map inter false
                            #pragma HLS dependence variable=temp_map inter false
        ...
        // Not enter in Loop length map
            ...
            if (read_data[j_tile] == 0) {
                // Loop length map
                LOOP_mapk_p0_expproj: for (DATA_HW k_tile = 0; k_tile < tile_index[2]; k_tile++) {
                    #pragma HLS LOOP_TRIPCOUNT min=7 max=28
                    LOOP_mapl_p0_expproj: for (DATA_HW l_tile = 0; l_tile < HLS_tile_map; l_tile++) {
                        #pragma HLS PIPELINE II=1
                        // CONV
                        if (l_tile < tile_index[2]) {
                            // Read in data
...
void PEs(hls::stream<DATA_STREAM> &ext_in_data, hls::stream<DATA_STREAM> &ext_out_data,
    ...
    if (call_PE < MAX_CALL[type_layer]) {
        if (type_layer == 0) {
            layer_CONV_3x3(ext_in_data, ext_out_data, ext_w_conv, ext_b_conv, ext_bn_scale, ext_bn_shift, tile_index, quant, info, type_layer);
        } else if (type_layer == 1) {
            layer_expansion_projection(ext_in_data, ext_out_data, ext_residual_map_read, ext_residual_map_write, ext_w_conv, ext_b_conv, ext_bn_scale, ext_bn_shift, tile_index, quant, info, type_layer);
        } else if (type_layer == 2) {
            layer_depthwise(ext_in_data, ext_out_data, ext_w_conv, ext_b_conv, ext_bn_scale, ext_bn_shift, tile_index, quant, info, type_layer);
        } else if (type_layer == 3) {
...
void MobileNet_Stream(hls::stream<DATA_STREAM> &ext_in_data, hls::stream<DATA_STREAM> &ext_out_data, hls::stream<DATA_STREAM> &ext_residual_map_write, hls::stream<DATA_STREAM> &ext_residual_map_read,
    ...
    #pragma HLS INTERFACE m_axi port=ext_bn_scale depth=bn_scale_layer offset=slave bundle=BN_SCALE
    #pragma HLS INTERFACE m_axi port=ext_bn_shift depth=bn_shift_layer offset=slave bundle=BN_SHIFT
    ...
    for (CALL_DATA i = 0; i < MAX_FC; i += stage_pipeline) {
        ...
        // PEs
        PEs(ext_in_data, ext_out_data, ext_residual_map_read, ext_residual_map_write, ext_w_conv, ext_b_conv, ext_bn_scale, ext_bn_shift, ext_w_fc, ext_b_fc, tile_index_0, quant, info_0, i, type_layer);
        PEs(ext_in_data, ext_out_data, ext_residual_map_read, ext_residual_map_write, ext_w_conv, ext_b_conv, ext_bn_scale, ext_bn_shift, ext_w_fc, ext_b_fc, tile_index_1, quant, info_1, i + 1, type_layer);
    }
}
